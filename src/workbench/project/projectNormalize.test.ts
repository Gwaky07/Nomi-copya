...
describe('extractCanvasThumbnailUrls（封面派生 + 无产物降级）', () => {
  it('从有产物的节点取前若干个 url（封顶 max）', () => {
    const nodes = [
      node({ id: 'a', result: { type: 'image', url: 'https://cdn/a.png' } as never }),
      node({ id: 'b', result: { type: 'video', thumbnailUrl: 'https://cdn/b.png' } as never }),
      node({ id: 'c', result: { type: 'image', url: 'https://cdn/c.png' } as never }),
    ]
  ...
  it('示例/空项目（节点无产物）返回明确的空标记 []（供 UI 占位）', () => {
    const nodes = [node({ id: 'empty-1' }), node({ id: 'empty-2', result: { type: 'image', url: '' } as never })]
  ...
  it('降级：数组里混入 null/非对象节点时跳过而不崩（与 main 侧派生等价的健壮性）', () => {
    const nodes = [
      ...
      undefined,
      'not-a-node',
      node({ id: 'good', result: { type: 'image', url: 'https://cdn/good.png' } as never }),
  ...
  it('过滤过短 url（length <= 4）', () => {
    const nodes = [
      node({ id: 'tiny', result: { type: 'image', url: 'abcd' } as never }),
      node({ id: 'ok', result: { type: 'image', url: 'abcde' } as never }),
    ]
    expect(extractCanvasThumbnailUrls(nodes)).toEqual(['abcde'])
  })

  it('视频只有 mp4 url、没有缩略图时，不把 mp4 当静态封面', () => {
    const nodes = [
      node({ id: 'video-only', result: { type: 'video', url: 'nomi-local://asset/project/video.mp4' } as never }),
      node({ id: 'image', result: { type: 'image', url: 'https://cdn/cover.png' } as never }),
    ]
    expect(extractCanvasThumbnailUrls(nodes)).toEqual(['https://cdn/cover.png'])
  })
})

describe('extractThumbnailUrlsFromRaw', () => {
  it('从 payload.generationCanvas.nodes 派生', () => {
    const raw = { payload: { generationCanvas: { nodes: [{ result: { type: 'image', url: 'https://cdn/x.png' } }] } } }
